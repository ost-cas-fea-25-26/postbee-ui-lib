...
export const Link: React.FC<LinkProps> = ({ className = '', children, href, target, rel, disabled = false, ...props }) => {
  ...
  const baseStyles =
    'text-primary text-sm underline decoration-1 underline-offset-2 focus-visible:ring-2 focus-visible:ring-primary-300';
  const hoverStyles = 'hover:decoration-primary-200';
  const disabledStyles = 'pointer-events-none opacity-40 !text-black';
  const transitionStyles = 'transition duration-300 ease-in-out';
  ...
  return (
    <a
      ...
      target={target}
      rel={safeRel}
      className={clsx(baseStyles, hoverStyles, transitionStyles, className)}
    >
      {children}
